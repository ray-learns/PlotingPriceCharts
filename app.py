...
# 1. PAGE SETUP
st.set_page_config(page_title="NIFTY 5 5-Day Forecaster", page_icon="🏛️", layout="wide")

st.title("🏛️ NIFTY 50 5-Day Forecast Analysis")
st.markdown("Select an ARIMA model to generate a **5-day forecast table** for NIFTY 50 stocks.")
...
def create_pdf(df, ticker_name, model_name, forecast_df, fig):
    ...
    try:
        ...
        p.drawImage(ImageReader(io.BytesIO(img_bytes)), 50, 420, width=500, height=250)
    except:
        p.drawString(100, 500, "Chart could not be rendered in PDF.")

    p.setFont("Helvetica-Bold", 14)
    p.drawString(100, 380, "Next 5-Day Forecast Table")
...
if not df.empty:
    ...
    fig.update_layout(template="plotly_dark", title=f"Historical Price: {selected_name}", xaxis_title="Date", yaxis_title="Closing Price (₹)", hovermode="x unified")
    st.plotly_chart(fig, use_container_width=True)

    # 7. FORECASTING (Next 5 Days Only)
    st.write("---")
    st.subheader(f"Next 5-Day Forecast ({model_choice})")
    ...
    with st.spinner(f"Running {model_choice}..."):
        try:
            ...
            # 8. REPORT GENERATION
            if st.button("Generate & Download 5-Day Report"):
                pdf_data = create_pdf(df, selected_name, model_choice, forecast_df, fig)
                st.download_button(
                    label="📥 Save PDF",
                    data=pdf_data,
                    file_name=f"{selected_name}_5Day_Forecast.pdf",
